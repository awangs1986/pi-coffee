...
try{
 ...
 await page.goto(`http://127.0.0.1:${web.address().port}`);await page.locator('#project-controls').waitFor({state:'visible'});await page.selectOption('#project-select',p.id);await page.click('#new-task');
 ...
 await page.getByText('浏览器测试数据：已检查当前工作区。').waitFor({timeout:12000});await page.click('#files-toggle');await page.getByRole('button',{name:'example.svg',exact:true}).click();
 ...
 console.log(JSON.stringify({errors,workspaceCount:(await workspaces.list()).conversations.length,preview:'SVG rendered and Markdown heading rendered'},null,2));
