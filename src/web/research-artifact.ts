...
export class ResearchArtifactStore {
  ...
  /** Refinement creates a new immutable evidence file; raw content never returns to Pi messages. */
  refine(ref: ResearchArtifactRef, conclusion: string): ResearchArtifactRef {
    const original = this.read(ref).split("\n## Refined conclusion\n")[0];
    const markdown = `${original}\n## Refined conclusion\n\n${redactSecrets(conclusion).slice(0, 2400)}\n`;
    const sha256 = createHash("sha256").update(markdown, "utf8").digest("hex");
    const artifactId = `research-${sha256.slice(0, 20)}`;
    const path = join(this.root, `${artifactId}.md`);
    ...
    writeFileSync(temporary, markdown, { encoding: "utf8", mode: 0o600 });
    renameSync(temporary, path);
    return { artifactId, path, sha256, bytes: Buffer.byteLength(markdown),
      pointer: `Research artifact ${artifactId} (sha256:${sha256.slice(0, 16)}; User VM path: ${path})` };
  }
...
export function pointerContext(ref: ResearchArtifactRef, conclusion: string): string {
  return `[Research sealed] ${ref.pointer}\nSummary and source index: ${redactSecrets(conclusion).trim().slice(0, 3200)}`.slice(0, 4096);
}
